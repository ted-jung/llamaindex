...
# Load it if not be loaded (load to build and persist for next)
if not index_loaded:
...
# Turn it into engine and transform it
...
def agent_input_fn(state: Dict[str, Any]) -> Dict[str, Any]:
...
# Create an FnAgentWorker and create task and run with pre-defined query pipeline
